...
def _get_bins_as_percentage_of_max(bins):
    ...
    acting_max_bin_energies = np.maximum(np.subtract(acting_max_bin_energies, 0.1), 0)
    ...
    acting_max_bin_energies = np.maximum(max_bin_energies_gate, acting_max_bin_energies)
...
def get_bins(
    ...
    logger.critical(f"channel: {channel}, id: {id}")
